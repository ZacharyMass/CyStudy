...
public class TermsController {
  ...
  /**
   * Endpoint to add a term to a class. Checks if the class exists, and if it does, adds the card.
   * We don't check if the topic exists, b/c it either didn't, or does now with this addition. That
   * will be fixed... later...
   *
  ...
  /**
   * Endpoint to get all of the terms for a specific topic in a specific class. Class name must be
   * specified b/c it is possible for multiple classes to have topics of the same name (e.g. Chapter
   * 1, etc.).
   *
   * @param className name of the class the topic belongs to
   * @param topic name of the topic
   * @return null if the topic doesn't exist, or the List of Terms objects (as a JSON if called via
   *     HTTP)
   */
  @GetMapping(path = "get-terms-by-topic")
  public @ResponseBody List<Terms> getTermsByTopic(
      @RequestParam String className, @RequestParam String topic) {
