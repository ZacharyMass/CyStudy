...
public class UserService {
    ...
    /**
     * Gets a specific user by name. This assumes the user is registered, as it's checked in the controller via calling
     * checkUserExists.
     *
    ...
    /**
     * Custom save function to save a user to the database. Custom so that we hash the password before adding the user's
    ...
    /**
     * A login function to ensure that the user logging in has matching credentials to one in the database.
     *
     * @param u User model containing only username and password that is sent from a login request
     * @return boolean depending on if username and password match the user's credentials in the database
     */
